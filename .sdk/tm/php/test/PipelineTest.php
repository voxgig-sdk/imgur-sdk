...
{
    private static function utility(): ImgurUtility
    {
        return new ImgurUtility();
    }
    ...
    private static function ctx(array $over = []): ImgurContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ImgurContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ImgurOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ImgurError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ImgurOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ImgurMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ImgurOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ImgurMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ImgurOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ImgurMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImgurSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ImgurMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ImgurResponse([]),
            'result' => new ImgurResult([]),
        ]);
        [, $err] = ImgurMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ImgurSpec(['step' => 's']),
            'response' => new ImgurResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ImgurResult([]),
        ]);
        [, $err] = ImgurMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ImgurSpec(['step' => 's']),
            'response' => new ImgurResponse(self::resp(200, ['v' => 1])),
            'result' => new ImgurResult([]),
        ]);
        [, $err] = ImgurMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ImgurSpec(['step' => 's']),
            'response' => new ImgurResponse(self::resp(200, ['v' => 2])),
            'result' => new ImgurResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ImgurMakeResponse::call($ctx);
    ...
    {
        $preset = new ImgurResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ImgurSpec([]),
            'response' => new ImgurResponse([]),
            'result' => new ImgurResult([]),
        ...
        [$got, $err] = ImgurMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ImgurResult([])]);
        [, $err] = ImgurMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ImgurSpec([]), 'result' => null]);
        [, $err] = ImgurMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImgurSpec(['step' => 's']),
            'result' => new ImgurResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ImgurOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImgurMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImgurSpec(['step' => 's']),
            'result' => new ImgurResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ImgurOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImgurMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImgurResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ImgurSpec([]),
            'result' => new ImgurResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ImgurMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new ImgurError('boom', 'boom');
        $utility->fetcher = function (ImgurContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImgurSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ImgurMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ImgurContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ImgurMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ImgurResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ImgurContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImgurSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ImgurMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ImgurResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ImgurSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ImgurMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ImgurMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ImgurMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ImgurResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ImgurDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ImgurResult(['ok' => false])]);
        $this->expectException(ImgurError::class);
        ImgurDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ImgurResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ImgurMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ImgurBaseFeature();
        $b = new ImgurBaseFeature();
        ImgurFeatureAdd::call($ctx, $a);
        ImgurFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ImgurBaseFeature
    {
        $f = new ImgurBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): ImgurContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ImgurSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new ImgurResponse(['headers' => null]),
            'result' => new ImgurResult([]),
        ]);
        ImgurResultHeaders::call($ctx);
