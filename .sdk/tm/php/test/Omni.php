...
//    only - a typed ImgurContext there would make every ctx
...
final class ImgurOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ImgurContext($clean, null);
    ...
    public static function err_from_map($m): ?ImgurError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ImgurError(is_string($code) ? $code : '', $msg);
    }
