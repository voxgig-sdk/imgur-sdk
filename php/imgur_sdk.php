...
class ImgurSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ImgurHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ImgurHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ImgurFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ImgurFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ImgurUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ImgurHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ImgurHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ImgurHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ImgurError($op . "_allow",
                "ImgurSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ImgurHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ImgurError("graphql_error",
                "ImgurSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function PostMeta($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_post_meta === null) {
                $this->_post_meta = new PostMetaEntity($this, null);
            }
            return $this->_post_meta;
        }
        return new PostMetaEntity($this, $data);
    }
