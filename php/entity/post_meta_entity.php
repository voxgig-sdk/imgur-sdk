...
class PostMetaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PostMetaEntity($this->_client, $opts);
    }

    /**
     * @param PostMeta|array $args PostMeta data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ImgurHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PostMeta|array The current PostMeta data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PostMeta fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ImgurHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PostMeta fields).
     */
    ...
    /**
     * List PostMeta items matching the given filter.
     *
     * @param PostMetaListMatch|array|null $reqmatch Match filter (any subset
     *   of PostMeta fields) as an assoc-array; PostMetaListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PostMeta[]|array A list of PostMeta items as assoc-arrays at
     *   the SDK boundary; throws ImgurError on failure (item-5 convention).
     */
