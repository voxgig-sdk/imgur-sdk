...
class FtCtrl extends ImgurControl
{
...
class FtClient
{
    ...
    private ImgurUtility $utility;

    public function __construct(ImgurUtility $utility, array $options)
    {
    ...
    public function get_utility(): ImgurUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ImgurContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ImgurContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ImgurConfig::make_feature($name);
    ...
    private static function buildUrl(ImgurSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ImgurError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ImgurResponse($fetched);
        }
    ...
    private function fail(ImgurContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ImgurContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
