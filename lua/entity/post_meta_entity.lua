...
local PostMetaEntity = {}
PostMetaEntity.__index = PostMetaEntity


function PostMetaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PostMetaEntity)
  self._name = "post_meta"
  self._client = client
...
function PostMetaEntity:get_name()
  return self._name
end


function PostMetaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PostMetaEntity.new(self._client, opts)
end
...
function PostMetaEntity:mark_deleted()
  self._deleted = true
end


function PostMetaEntity:deleted()
  return true == self._deleted
end


function PostMetaEntity:data_set(args)
  if args ~= nil then
...
function PostMetaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PostMetaEntity:match_set(args)
  if args ~= nil then
...
function PostMetaEntity:match_get()
...
function PostMetaEntity:stream(action, args, callopts)
...
---@param reqmatch PostMetaListMatch
---@param ctrl? table
---@return PostMeta[]
---@return string? err
function PostMetaEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PostMetaEntity:_run_op(ctx, post_done)
  local utility = self._utility
