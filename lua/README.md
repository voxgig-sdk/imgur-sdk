...
The Lua SDK for the Imgur API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Image(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Imgur(nil):load(
  { id = "test01" }, nil
)
