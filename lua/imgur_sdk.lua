...
ImgurSDK._make_feature = _make_feature


function ImgurSDK.new(options)
  local self = setmetatable({}, ImgurSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ImgurSDK:get_utility()
  return Utility.copy(self._utility)
end


function ImgurSDK:get_root_ctx()
  return self._rootctx
end


function ImgurSDK:prepare(fetchargs)
...
function ImgurSDK:direct(fetchargs)
...
-- by passing it.
function ImgurSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ImgurSDK:graphql(query, variables, ctrl)
...
-- Idiomatic facade: client:PostMeta():list() / client:PostMeta():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function ImgurSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ImgurSDK.new(sdkopts)
  sdk.mode = "test"
