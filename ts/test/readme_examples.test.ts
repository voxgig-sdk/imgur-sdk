...
//     (\`new ImgurSDK(...)\` / \`ImgurSDK.test(...)\` become
//     \`ImgurSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"image":{"test01":{"id":"test01"}},"post_meta":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ImgurSDK(...)\` and any
// \`ImgurSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ImgurSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ImgurSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
