...
describe('PostMetaDirect', async () => {
  ...
  test('direct-list-post_meta', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-post_meta', setup.live)) return
    if (skipIfMissingIds(t, setup, ["post_meta01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IMGUR_TEST_POST_META_ENTID']
