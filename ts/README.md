# Imgur TypeScript SDK



The TypeScript SDK for the Imgur API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Image()` — each with a small set of operations (`list`, `load`)
...
```ts
try {
  const image = await client.Image().load({ id: 'example_id' })
  console.log(image)
} catch (err) {
...
| `tester(testopts?, sdkopts?)` | `ImgurSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ImgurSDK.test(testopts?, sdkopts?)` | `ImgurSDK` | Create a test-mode client. |
...
| `client` | `client(): ImgurSDK` | Return the parent SDK client. |
