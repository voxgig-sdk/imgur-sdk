// Command Imgur-cli is a CLI + REPL for the Imgur SDK,
...
const entitiesHelp = "image post_meta"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IMGUR_APIKEY (api key), IMGUR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IMGUR_APIKEY carries the API key and
	// IMGUR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IMGUR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IMGUR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewImgurSDK(opts)
