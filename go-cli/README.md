...
boru-driven command-line client **and** interactive REPL for the Imgur
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/imgur-cli)
make build
...
./imgur-cli load 1 image            # {id:1} shorthand
./imgur-cli load '{id:1}' image       # explicit match map

# 5. Override the API base URL for a single call
IMGUR_BASE=https://api.example.com ./imgur-cli load 1 image
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/imgur-cli
   ```
...
```sh
./imgur-cli load 1 image          # scalar shorthand for {id:1}
./imgur-cli load '{id:1}' image     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `imgur>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/imgur-cli`. |
...
(`./imgur-cli <expr>`) and interactively (the REPL), and expressions compose the
