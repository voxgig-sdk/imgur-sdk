...
class PostMetaEntity
  ...
  def make
    opts = @_entopts.dup
    PostMetaEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ImgurHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PostMeta, Hash] the current PostMeta data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ImgurHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PostMeta fields)
  def match_get
  ...
  #
  # @param reqmatch [PostMetaListMatch, Hash, nil] match filter (any subset of
  #   PostMeta fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PostMeta>, Array] the matching PostMeta items; raises ImgurError on failure
