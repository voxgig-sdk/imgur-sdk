...
import (
	...
	omni "github.com/voxgig-sdk/imgur-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ImgurError); is {
				out := map[string]any{
					"name":    "ImgurError",
...
func omniCtx(arg any, client *sdk.ImgurSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ImgurSDK); is {
			client = live
