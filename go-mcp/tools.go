...
type Args struct {
	Entity string         `json:"entity" jsonschema:"image | post_meta"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ImgurSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "imgur_list",
		Description: "List records from Imgur. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "imgur_load",
		Description: "Load a single record from Imgur. " +
...
func runOp(client *sdk.ImgurSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ImgurSDK, name string) (sdk.ImgurEntity, error) {
	switch strings.ToLower(name) {
	case "image":
		return client.Image(nil), nil
	case "post_meta":
		return client.PostMeta(nil), nil
