...
from imgur_sdk.imgur_types import (
    PostMeta,
    PostMetaListMatch,
)
...
class PostMetaEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PostMetaEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PostMeta]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PostMeta().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
