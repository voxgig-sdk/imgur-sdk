...
class ImgurTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ImgurTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ImgurTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ImgurTestRunner.getenv("IMGUR_TEST_LIVE")
        override = ImgurTestRunner.getenv("IMGUR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ImgurTestRunner.getenv(key)
        ...
        explain = ImgurTestRunner.getenv("IMGUR_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IMGUR_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ImgurTestRunner._test_control is not None:
            return ImgurTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ImgurTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ImgurTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ImgurTestRunner.load_test_control()
...
def load_env_local():
    ImgurTestRunner.load_env_local()


def env_override(m):
    return ImgurTestRunner.env_override(m)


def entity_data(v):
    return ImgurTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ImgurTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ImgurTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ImgurTestRunner.load_test_control()


def live_client_options():
    return ImgurTestRunner.live_client_options()


def live_delay_ms():
    return ImgurTestRunner.live_delay_ms()
